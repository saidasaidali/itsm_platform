    ...
    scanQrCode(token)
      .then((res) => {
        if (res.success) {
          ...
          setError(res.message || 'Équipement introuvable pour ce QR Code.')
        }
      })
      .catch((err) => {
        ...
        setError('QR Code invalide ou équipement introuvable.')
      })
  ...
  if (loading) {
    return (
      <div className="text-center p-5">
        <CSpinner size="lg" />
        <p className="mt-3 text-muted">Scan en cours...</p>
      </div>
    )
  }
      ...
      <CRow className="justify-content-center mt-5">
        <CCol lg={6}>
          <CAlert color="warning">
            ...
            Accès non autorisé. Vous n'avez pas les permissions nécessaires pour accéder à cette ressource.
          </CAlert>
    ...
    <CRow className="justify-content-center">
      <CCol lg={7}>
        ...
        <div className="text-center mb-4 mt-2">
          ...
          <h5 className="fw-bold mb-1">Équipement scanné</h5>
          <p className="text-muted small mb-0">
            Informations de l'équipement associé au QR Code
          </p>
        </div>
        ...
        <CCard className="mb-4">
          ...
          <CCardBody>
            <CRow className="g-3">
              <CCol md={6}>
                <div className="small text-muted mb-1">Marque / Modèle</div>
                <div className="fw-semibold">{asset.brand} {asset.model}</div>
              </CCol>
              <CCol md={6}>
                <div className="small text-muted mb-1">Salle / Emplacement</div>
                <div className="fw-semibold">{asset.location || '—'}</div>
              </CCol>
              <CCol md={6}>
                <div className="small text-muted mb-1">Utilisateur affecté</div>
                <div className="fw-semibold">{asset.assigned_to_name || '—'}</div>
              </CCol>
              <CCol md={6}>
                <div className="small text-muted mb-1">Numéro de série</div>
                <div className="fw-semibold">{asset.serial_number || '—'}</div>
              </CCol>
              {asset.department && (
                <CCol md={6}>
                  <div className="small text-muted mb-1">Département</div>
            ...
            <div className="d-flex align-items-center gap-2 text-muted small">
              ...
              {asset.scan_count} scan(s) enregistré(s) pour cet équipement
            </div>
          </CCardBody>
        </CCard>
