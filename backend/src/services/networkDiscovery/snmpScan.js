...
async function processDevice(data) {
  ...
  const { rows } = await pool.query(
    `SELECT id FROM assets WHERE adresse_ip = $1 OR numero_serie_fabricant = $2 LIMIT 1`,
  ...
  // Déterminer le type approximatif depuis la description SNMP
  ...
  const { rows: created } = await pool.query(
    `INSERT INTO assets
       (asset_tag, type, brand, model, status, adresse_ip,
        numero_serie_fabricant, location, last_seen_at, discovery_method)
  ...
  for (const admin of admins) {
    await pool.query(
      ...
      ['🆕 Équipement réseau détecté', `${type} "${assetTag}" (${ip}) ajouté automatiquement.`, admin.id, created[0].id]
    );
  }
...
export async function runSNMPScan(baseIp, start = 1, end = 254) {
  ...
  const results = { created: 0, updated: 0, scanned: 0 };

  // Scan par lots de 20 pour ne pas saturer le réseau
  const batchSize = 20;
  ...
  console.log(`[SNMPScan] ✅ Terminé — ${results.scanned} IP scannées, ${results.created} créés, ${results.updated} mis à jour.`);
  return results;
}
