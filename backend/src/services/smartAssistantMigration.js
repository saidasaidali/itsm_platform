// backend/src/services/smartAssistantMigration.js
// Migration automatique pour les tables du Smart Assistant
import pool from '../db.js';

export async function runSmartAssistantMigration() {
  try {
    console.log('[Migration] Vérification/création des tables Smart Assistant...');

    // 1. Créer la table smart_assistant_logs
    await pool.query(`
    ...
    // 2. Créer la table security_incidents
    await pool.query(`
    ...
    // 3. Créer la table smart_assistant_metrics
    await pool.query(`
    ...
    // 6. Créer la fonction et le trigger pour les métriques
    await pool.query(`
    ...
    console.log('[Migration] ✅ Toutes les tables Smart Assistant ont été créées/vérifiées');
    return { success: true, message: 'Migration Smart Assistant terminée' };

  } catch (error) {
    console.error('[Migration] ❌ Erreur lors de la migration Smart Assistant:', error.message);
    throw error;
  }
}

export default {
  runSmartAssistantMigration
};