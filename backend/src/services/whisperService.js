...
class WhisperService {
  constructor() {
    // Chemin vers whisper-cli.exe (depuis .env)
    this.whisperPath = process.env.WHISPER_CPP_PATH || 'whisper-cli';

    // Chemin modèle : si relatif, le résoudre depuis la racine du projet backend
    const rawModel = process.env.WHISPER_MODEL_PATH || './models/ggml-base.bin';
    this.modelPath = path.isAbsolute(rawModel)
      ? rawModel
      : path.resolve(process.cwd(), rawModel);
  }
  ...
  async checkAvailability() {
    try {
      ...
      throw new Error(
        ...
        `Vérifiez WHISPER_CPP_PATH dans votre .env`
      );
    }
    try {
