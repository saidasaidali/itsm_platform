...
import { createCalendarEventFromTicket } from '../services/calendarSmartAssistant.js';
import { notifyTicketUpdated } from '../services/calendarSyncService.js';
...
export const createTicket = asyncHandler(async (req, res) => {
  ...
  if (asset_id) {
    ...
    // Notifier les clients SSE de la création du ticket
    notifyTicketUpdated({ ...ticket, action: 'created' });
    ...
    // ── Créer automatiquement un événement calendrier ───────────
    createCalendarEventFromTicket(ticket).catch(err => {
      console.error('[Calendar] Erreur création événement automatique:', err.message);
    });
...
export const updateStatus = asyncHandler(async (req, res) => {
  ...
  const ticket = await findTicketOrFail(id, req, res);
    ...
    if (newStatus === 'Résolu') {
      await smartAssistantService.learnFromTicket(id);
    ...
    // Notifier les clients SSE du changement de statut
    notifyTicketUpdated({ ...updatedTicket, action: 'status_updated', previous_status: ticket.status });
...
export const assignTicket = asyncHandler(async (req, res) => {
  ...
  const ticket = await findTicketOrFail(id, req, res);
    ...
    // Notifier les clients SSE de l'assignation
    notifyTicketUpdated({ id: ticket.id, assigned_to: technicianId, status: 'Assigné', action: 'assigned', technician_username: techRows[0].username });
...
export const transferTicket = asyncHandler(async (req, res) => {
  ...
  const ticket = await findTicketOrFail(id, req, res);
    ...
    // Notifier les clients SSE du transfert
    notifyTicketUpdated({ id: ticket.id, assigned_to: technicianId, action: 'transferred', technician_username: techRows[0].username });
...
export const deleteTicket = asyncHandler(async (req, res) => {
  ...
  if (!validateId(id, req, res)) return;
    ...
    // Notifier les clients SSE qu'un ticket a été supprimé
    notifyTicketUpdated({ id: parseInt(id), status: 'deleted', action: 'deleted' });
