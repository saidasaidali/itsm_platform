// Script de test standalone de la correction learnedCasesProvider
...
// Fonction de recherche simplifiée (copie de la version corrigée)
async function searchLearnedCases(query, limit = 3) {
  ...
  try {
    ...
    return rows.map(row => {
      // Calculer un score de pertinence basé sur la correspondance des keywords
      const caseKeywords = row.problem_keywords || [];
      const matchingKeywords = keywords.filter(kw => 
        caseKeywords.some(ck => ck.toLowerCase().includes(kw.toLowerCase()) || kw.toLowerCase().includes(ck.toLowerCase()))
      );
      
      // Score de pertinence = ratio de keywords qui matchent
      ...
      return {
        ...
        score: relevanceScore, // Score de pertinence, pas de confiance
...
async function testFix() {
  ...
  console.log('TEST DE LA CORRECTION - learnedCasesProvider (standalone)');
  console.log('═'.repeat(80));
