...
def build_asset_dataset(for_training=False):
    """
    Construit le dataset principal par asset pour entraînement et scoring.
    Chaque ligne = un asset avec ses features agrégées.

    Paramètre for_training :
      - False (défaut) : utilisation normale (scoring en temps réel)
      - True           : mode entraînement — le failure_label regarde dans le passé
                         (ticket Haute créé dans les 30 jours suivant la création de l'asset)
    """
    conn = get_connection()
    
    if for_training:
        # Mode entraînement : failure_label regarde dans le passé
        # On vérifie si un ticket Haute a été créé dans les 30 jours suivant la création de l'asset
        query = """
            SELECT
                a.id                                        AS asset_id,
                a.type,
                ...
                -- Label : panne dans les 30 jours après la création de l'asset
                -- (1 si un ticket Haute priorité a été créé dans les 30j suivant a.created_at)
                CASE WHEN EXISTS (
            ...
            GROUP BY a.id, a.type, a.status, a.purchase_date, a.created_at,
                     ls.cpu_usage, ls.ram_usage, ls.disk_free_gb,
                     ls.disk_total_gb, ls.uptime_hours, ls.is_online
        """
    else:
        # Mode scoring (inchangé) : failure_label regarde dans le futur
        query = """
            SELECT
                a.id                                        AS asset_id,
                a.type,
